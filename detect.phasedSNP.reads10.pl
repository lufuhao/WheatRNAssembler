...
open (CLUSTERLOG, ">$cluster_log") || die "(Main1)Error: can not write cluster log: $cluster_log\n";
# output fpkm calculated
close FPKMLOG if (defined fileno(FPKMLOG));
open (FPKMLOG, ">$fpkm_log") || die "(Main1)Error: can not write FPKM log: $fpkm_log\n";
# output allele log
close ALLELELOG if (defined fileno(ALLELELOG));
open (ALLELELOG, ">$allele_log") || die "(Main1)Error: can not write allele log: $allele_log\n";
...
##COMMENT: extract fasta reference
	...
	unless ( -s "$RunDir/Clust$cluster_num/ref.$cluster_num.fa" and "$RunDir/Clust$cluster_num/ref.$cluster_num.fa.fai") {
		if (&CdbYank($fasta_index, "$RunDir/Clust$cluster_num/ref.$cluster_num.fa", \@cluster_seqids, $path_cdbyank) == $cmd_failure_code) {
		...
		if (&IndexFasta("$RunDir/Clust$cluster_num/ref.$cluster_num.fa", $path_samtools) == $cmd_failure_code) {
...
##COMMENT: Extract bam from @bam_AABBDD, @bam_AABB, @bam_AA, @bam_DD
	...
	if (! &GetBam(\@bam_AABBDD, \@cluster_seqids, "$RunDir/Clust$cluster_num/AABBDD.$cluster_num.group.bam", "$RunDir/Clust$cluster_num/AABBDD.$cluster_num.nogroup.bam")) {
		print STDERR "(Main3)Error: AABBDD BAM extraction failed\n";
		print CLUSTERLOG $cluster_num."\tFail\t1\tGetBamAABBDD\n";
		next;
	}
	if (! &ReduceReadNameLength("$RunDir/Clust$cluster_num/AABBDD.$cluster_num.group.bam", "$RunDir/Clust$cluster_num/AABBDD.$cluster_num.group.RRNL.bam", $path_samtools)) {
		print STDERR "(Main12)Error: RRNL\n";
		print CLUSTERLOG $cluster_num."\tFail\t1\tRRNL\n";
		next;
	}
	### AABBDD FPKM
	...
	if (defined $file_vcf_aabbdd and -s $file_vcf_aabbdd) {
		if (! &ExtractVcf($file_vcf_aabbdd, $cluster_line, "$RunDir/Clust$cluster_num/AABBDD.$cluster_num.vcf.gz")) {
		...
		unless (-s "$RunDir/Clust$cluster_num/AABBDD.$cluster_num.vcf.gz" and -s "$RunDir/Clust$cluster_num/AABBDD.$cluster_num.vcf.gz.tbi") {
			if (! &RunFreebayes("$RunDir/Clust$cluster_num/ref.$cluster_num.fa", \@cluster_seqids, "$RunDir/Clust$cluster_num/AABBDD.$cluster_num.nogroup.bam", 3, "$RunDir/Clust$cluster_num/AABBDD.$cluster_num.vcf.gz", 0, $freebayes_additional_cmd, 0, 0, 1, 200, $path_freebayes, $path_freebayesparallel, $path_vcfsort, $path_vcfconcat, $path_tabix, $path_bgzip, $path_samtools)) {
				print STDERR "(Main3)Error: freebayes running AABBDD failed\n";
				print CLUSTERLOG $cluster_num."\tFail\t1\tFreeBayesAABBDD\n";
				next;
			}
		}
	}
	unless (-s "$RunDir/Clust$cluster_num/AABBDD.$cluster_num.vcf") {
		if (! &exec_cmd_return("gunzip -c $RunDir/Clust$cluster_num/AABBDD.$cluster_num.vcf.gz > $RunDir/Clust$cluster_num/AABBDD.$cluster_num.vcf")) {
			print STDERR "(Main3)Error: gunzip uncompress $RunDir/Clust$cluster_num/AABBDD.$cluster_num.vcf.gz failed\n";
			print CLUSTERLOG $cluster_num."\tFail\t1\tFreeBayesAABBDDgunzip\n";
			next;
		}
		elsif (! -s "$RunDir/Clust$cluster_num/AABBDD.$cluster_num.vcf") {
			print STDERR "(Main3)Error: gunzip output $RunDir/Clust$cluster_num/AABBDD.$cluster_num.vcf.gz failed\n";
			print CLUSTERLOG $cluster_num."\tFail\t1\tFreeBayesAABBDDgunzip_output\n";
			next;
		}
	}
	(my $test_readvcf_aabbdd, $aabbdd_vcf_obj)=&ReadVcf("$RunDir/Clust$cluster_num/AABBDD.$cluster_num.vcf.gz", $vcfqual);
	if (! $test_readvcf_aabbdd) {
		print STDERR "(Main3)Error: Reading AABBDD VCF failed\n";
		print CLUSTERLOG $cluster_num."\tFail\t1\treadvcfAABBDD\n";
		next;
	}
	...
	if ($debug) {
		print "$cluster_num: no vaiations detected, direct assembly\n";
	}
...
###BLOCKAABB
	...
	if (! &GetBam(\@bam_AABB, \@cluster_seqids, "$RunDir/Clust$cluster_num/AABB.$cluster_num.group.bam", "$RunDir/Clust$cluster_num/AABB.$cluster_num.nogroup.bam")) {
		print STDERR "(Main4)Error: AABB BAM extraction failed\n";
		print CLUSTERLOG $cluster_num."\tFail\t1\tGetBamAABB\n";
		next;
	}
	### AABB FPKM
	(my $TEtestcode_aabb, $test_aabb_expressed)=&TestExpressed("$RunDir/Clust$cluster_num/AABB.$cluster_num.nogroup.bam", $totalreads[1]);
	if ($TEtestcode_aabb) {
		print STDERR "(Main4)Error: Calculate AABB FPKM error\n";
		print CLUSTERLOG $cluster_num."\tFail\t1\tTestExpressAABB\n";
		next;
	}
	if ($test_aabb_expressed ==1) {
		#run freebayes for AABB
		if (defined $file_vcf_aabb and -s $file_vcf_aabb) {
			if (! &ExtractVcf($file_vcf_aabb, $cluster_line, "$RunDir/Clust$cluster_num/AABB.$cluster_num.vcf.gz")) {
				print STDERR "(Main4)Error: AABB VCF extraction failed\n";
				print CLUSTERLOG $cluster_num."\tFail\t1\tVcfExtractAABB\n";
				next;
			}
		}
		else {
			unless (-s "$RunDir/Clust$cluster_num/AABB.$cluster_num.vcf.gz" and -s "$RunDir/Clust$cluster_num/AABB.$cluster_num.vcf.gz.tbi") {
				if (! &RunFreebayes("$RunDir/Clust$cluster_num/ref.$cluster_num.fa", \@cluster_seqids, "$RunDir/Clust$cluster_num/AABB.$cluster_num.nogroup.bam", 2, "$RunDir/Clust$cluster_num/AABB.$cluster_num.vcf.gz", "$RunDir/Clust$cluster_num/AABBDD.$cluster_num.vcf.gz", $freebayes_additional_cmd, 0, 0, 1, 200, $path_freebayes, $path_freebayesparallel, $path_vcfsort, $path_vcfconcat, $path_tabix, $path_bgzip, $path_samtools)) {
					print STDERR "(Main4)Error: freebayes running AABB failed\n";
					print CLUSTERLOG $cluster_num."\tFail\t1\tFreeBayesAABB\n";
					next;
				}
			}
		}
		(my $test_readvcf_aabb, $aabb_vcf_obj)=&ReadVcf("$RunDir/Clust$cluster_num/AABB.$cluster_num.vcf.gz", $vcfqual);
		if (! $test_readvcf_aabb) {
			print STDERR "(Main4)Error: Reading AABB VCF failed\n";
			print CLUSTERLOG $cluster_num."\tFail\t1\treadvcfAABB\n";
			next;
		}
	}}###BLOCKAABB
...
###BLOCKAA
	BLOCKAA: {
	print "\n(Main5)Step: AA BAMs\n"; ### For test ###
	if (! &GetBam(\@bam_AA, \@cluster_seqids, "$RunDir/Clust$cluster_num/AA.$cluster_num.group.bam", "$RunDir/Clust$cluster_num/AA.$cluster_num.nogroup.bam")) {
		print STDERR "(Main5)Error: AA BAM extraction failed\n";
		print CLUSTERLOG $cluster_num."\tFail\t1\tGetBamAA\n";
		next;
	}
	### AA FPKM
	(my $TEtestcode_aa, $test_aa_expressed)=&TestExpressed("$RunDir/Clust$cluster_num/AA.$cluster_num.nogroup.bam", $totalreads[2]);
	if ($TEtestcode_aa) {
		print STDERR "(Main5)Error: Calculate AA FPKM error\n";
		print CLUSTERLOG $cluster_num."\tFail\t1\tTestExpressAA\n";
		next;
	}
	if ($test_aa_expressed ==1) {
		#run_freebayes for AA
		if (defined $file_vcf_aa and -s $file_vcf_aa) {
			if (! &ExtractVcf($file_vcf_aa, $cluster_line, "$RunDir/Clust$cluster_num/AA.$cluster_num.vcf.gz")) {
				print STDERR "(Main5)Error: AA VCF extraction failed\n";
				print CLUSTERLOG $cluster_num."\tFail\t1\tVcfExtractAA\n";
				next;
			}
		}
		else {
			unless (-s "$RunDir/Clust$cluster_num/AA.$cluster_num.vcf.gz" and -s "$RunDir/Clust$cluster_num/AA.$cluster_num.vcf.gz.tbi") {
				if (! &RunFreebayes("$RunDir/Clust$cluster_num/ref.$cluster_num.fa", \@cluster_seqids, "$RunDir/Clust$cluster_num/AA.$cluster_num.nogroup.bam", 2, "$RunDir/Clust$cluster_num/AA.$cluster_num.vcf.gz", "$RunDir/Clust$cluster_num/AABBDD.$cluster_num.vcf.gz", $freebayes_additional_cmd, 0, 0, 1, 200, $path_freebayes, $path_freebayesparallel, $path_vcfsort, $path_vcfconcat, $path_tabix, $path_bgzip, $path_samtools)) {
					print STDERR "(Main5)Error: freebayes running AA failed\n";
					print CLUSTERLOG $cluster_num."\tFail\t1\tFreeBayesAA\n";
					next;
				}
			}
		}
		(my $test_readvcf_aa, $aa_vcf_obj)=&ReadVcf("$RunDir/Clust$cluster_num/AA.$cluster_num.vcf.gz", $vcfqual);
		if (! $test_readvcf_aa) {
			print STDERR "(Main5)Error: Reading AA VCF failed\n";
			print CLUSTERLOG $cluster_num."\tFail\t1\treadvcfAA\n";
			next;
		}
	}}###BLOCKAA
...
###BLOCKDD
	BLOCKDD: {
	print "\n(Main6)Step: DD BAMs\n"; ### For test ###
	if (! &GetBam(\@bam_DD, \@cluster_seqids, "$RunDir/Clust$cluster_num/DD.$cluster_num.group.bam", "$RunDir/Clust$cluster_num/DD.$cluster_num.nogroup.bam")) {
		print STDERR "(Main6)Error: DD BAM extraction failed\n";
		print CLUSTERLOG $cluster_num."\tFail\t1\tGetBamDD\n";
		next;
	}
	(my $TEtestcode_dd, $test_dd_expressed)=&TestExpressed("$RunDir/Clust$cluster_num/DD.$cluster_num.nogroup.bam", $totalreads[3]);
	if ($TEtestcode_dd) {
		print STDERR "(Main6)Error: Calculate DD FPKM error\n";
		print CLUSTERLOG $cluster_num."\tFail\t1\tTestExpressDD\n";
		next;
	}
	if ($test_dd_expressed ==1) {
	#run_freebayes for DD
		if (defined $file_vcf_dd and -s $file_vcf_dd) {
			if (! &ExtractVcf($file_vcf_dd, $cluster_line, "$RunDir/Clust$cluster_num/DD.$cluster_num.vcf.gz")) {
				print STDERR "(Main6)Error: DD VCF extraction failed\n";
				print CLUSTERLOG $cluster_num."\tFail\t1\tVcfExtractDD\n";
				next;
			}
		}
		else {
			unless (-s "$RunDir/Clust$cluster_num/DD.$cluster_num.vcf.gz" and -s "$RunDir/Clust$cluster_num/DD.$cluster_num.vcf.gz.tbi") {
### &RunFreebayes($ref.fasta, \@seqids, $bam, $ploidy, $output.vcf.gz, $guided.vcf/0, $additions_cmd, [$run_parallel(1/0)], [Parallel_region file], [$threads], [$fastabin], [path_freebayes], [path_freebayesparallel], [path_vcfsort], [path_concat], [path_tabix], [path_bgzip], [path_samtools])
				if (! &RunFreebayes("$RunDir/Clust$cluster_num/ref.$cluster_num.fa", \@cluster_seqids, "$RunDir/Clust$cluster_num/DD.$cluster_num.nogroup.bam", 2, "$RunDir/Clust$cluster_num/DD.$cluster_num.vcf.gz", "$RunDir/Clust$cluster_num/AABBDD.$cluster_num.vcf.gz", $freebayes_additional_cmd, 1, 0, 1, 0, $path_freebayes, $path_freebayesparallel, $path_vcfsort, $path_vcfconcat, $path_tabix, $path_bgzip, $path_samtools)) {
					print STDERR "(Main6)Error: freebayes running DD failed\n";
					print CLUSTERLOG $cluster_num."\tFail\t1\tFreeBayesDD\n";
					next;
				}
			}
		}
		(my $test_readvcf_dd, $dd_vcf_obj)=&ReadVcf("$RunDir/Clust$cluster_num/DD.$cluster_num.vcf.gz", $vcfqual);
		if (! $test_readvcf_dd) {
			print STDERR "(Main6)Error: Reading DD VCF failed\n";
			print CLUSTERLOG $cluster_num."\tFail\t1\treadvcfDD\n";
			next;
		}
	}}###BLOCKDD
...
##COMMENT: Group alleles into subgenome
	...
	my ($test_readbam, $aabbdd_bam_obj)=&ReadSam("$RunDir/Clust$cluster_num/AABBDD.$cluster_num.group.RRNL.bam", "Clust$cluster_num/ref.$cluster_num.fa", 1);
	unless ($test_readbam) {
	...
	if (1) {
		foreach my $idvchrom (keys %{$rgfpkms}) {
			if (exists ${$rgfpkms}{$idvchrom}{'chr'}) {
				print FPKMLOG "Clust$cluster_num\t$idvchrom\tChr\t".${$rgfpkms}{$idvchrom}{'chr'}."\n";
			}
			if (exists ${$rgfpkms}{$idvchrom}{'allelic'}) {
				foreach (sort (keys %{${$rgfpkms}{$idvchrom}{'allelic'}})) {
					print FPKMLOG "Clust$cluster_num\t$idvchrom\tAllelic\t$_\t".${$rgfpkms}{$idvchrom}{'allelic'}{$_}."\n";
				}
			}
			if (exists ${$rgfpkms}{$idvchrom}{'read_group'}) {
				foreach my $ind_rg (sort (keys %{${$rgfpkms}{$idvchrom}{'read_group'}})) {
					print FPKMLOG "Clust$cluster_num\t$idvchrom\tReadGroup\t$ind_rg";
...
#				print "Chr $BFQchrom FPKM: ${$BFQfpkms{$BFQchrom}}{'chr'}\n";### For test ###
			}
			else {
