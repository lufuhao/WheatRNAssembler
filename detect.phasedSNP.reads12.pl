...
#							)
#					);
	(my $test_groupvcf, my $rnaseq_allele_hashindex, my $fragments_hashindex, my $readsum, my $readid_alleles_hashindex, $test_bb_expressed)=&GroupVcf($aabbdd_bam_obj, $aabbdd_vcf_obj);
	if ($test_groupvcf) {
...
##Format: $fixedGeno_4fillin{chr}->{pos}='0/1/0'; ('A/B/D')
	...
	if ($ploidy==1) {
		...
		(my $test_fiv, $test_run_phase, my $fixedgeno_hashindex, my $fillinAlleles_hashindex)=&FillInVariations(\%FixedAllele_2genome,$aabbdd_vcf_obj, $ploidy, %{$readsum});
...
#			${$GVshare_readIDs{$GVchrom}}{$GVind_align->name}++;
			...
			$GVreadidsum{$GVind_align->name}{'ref'}{$GVchrom}++;
