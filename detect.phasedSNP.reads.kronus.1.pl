...
use strict;
use warnings;
#use lib '/usr/users/celldev/luf/bin/perl5lib/20151105';
use Cwd;
use FindBin qw($Bin);
use Getopt::Long;
use Bio::DB::Sam;
...
use FuhaoPerl5Lib::VcfKit qw/ExtractVcf ReadVcf ReadVariantType HapcompassVcf HcFragments RunHapCompass GroupFragments CorrectAlleles ReadHcOut/;
...
Options:
	...
	--bam	<bam.list file>
	--refvcf <AABBDD.VCF.gz.file>
	--refallele <AABBDD.allele file>
	--refploidy <AABBDD.ploidy file>
	--vcf	<vcf.gz file>
	--allele	<Allele configure file>
	--numreads	<file_totalreads>
	--taggenome	<[Opt] Genome tag in AABB BAM, default: zg>
	--tagchrom	<[Opt] Chromosome tag in AABB BAM, default: ac>
...
my ($file_bam);
my ($file_vcf, $file_alleles, $file_refvcf, $file_refallele, $file_ploidy);
...
GetOptions(
	...
	"bam:s" => \$file_bam,
	"vcf:s" => \$file_vcf, 
	"refvcf:s" => \$file_refvcf,
	"refallele:s" => \$file_refallele,
	"refploidy:s" => \$file_ploidy,
#	"fpkm:s" => \$file_fpkm,
...
die "Please specify the file for bam list/bam\n" unless (defined $file_bam and -s $file_bam);
...
die "Please specify the file for REF alleles\n" unless (defined $file_refallele and -s $file_refallele);
#die "Please specify the file for FPKM\n" unless (defined $file_fpkm and -s $file_fpkm);
die "Please specify the file for AABBDD ploidy\n" unless (defined $file_ploidy and -s $file_ploidy);
...
die "InputOutputError: empty bam list\n" if (scalar(@inputbams)<1);
print "InputOutputReport: Reading bam list: ".scalar(@inputbams)."\n" if ($debug or $verbose);
foreach (@inputbams) {
	print "---> $_\n" if ($debug or $verbose);
	unless (-s "$_.bai") {
...
$bamfiles=join(',', @inputbams);
$numbams=scalar(@inputbams);
undef @inputbams;
...
if (defined $file_alleles and -s $file_alleles) {
	...
	while (my $line=<EXISTINGALLELE>) {
		chomp $line;
		my @arr=split(/\t/, $line);
		${$refallelesABD}{$arr[0]}{$arr[1]}{'A'}=$arr[2];
#		${$refallelesABD}{$arr[0]}{$arr[1]}{'B'}='?';#$arr[3];
#		${$refallelesABD}{$arr[0]}{$arr[1]}{'D'}=$arr[4];
	}
	close EXISTINGALLELE;
}

###format: %genomealleles{chr}{$pos}=('A' => 0/1, 'B' => 1/0, 'D' => 1/0)
my %genomealleles=();
if (defined $file_refallele and -s $file_refallele) {

	close GENALLELE if (defined fileno(GENALLELE));
	unless (open (GENALLELE, "< $file_alleles")) {
		die "Error: can not read genomic allele file\n";
	}
	while (my $line=<GENALLELE>) {
		chomp $line;
		my @arr=split(/\t/, $line);
		$genomealleles{$arr[0]}{$arr[1]}{'A'}=$arr[2];
		$genomealleles{$arr[0]}{$arr[1]}{'B'}=$arr[3];
	}
	close GENALLELE;
}
...
if (defined $file_vcf) {
	if (! -s $file_vcf) {
		die "MainError: can not find VCF file: $file_vcf\n";
	}
	if ($file_vcf=~/\.vcf$/i) {
		if (! BgzipVcf($file_vcf, "$file_vcf.gz", $path_bgzip)) {
			die "InputOutputError: VCF bgzip error\n";
		}
		elsif (! IndexVcf("$file_vcf.gz", $path_tabix)) {
			die "InputOutputError: VCF index error\n";
		}
		else {
			$file_vcf.='.gz';
		}
		...
		unless (-s "$file_vcf.tbi") {
			unless (IndexVcf($file_vcf, $path_tabix)) {
				die "InputOutputError: VCF.gz index error\n";
			}
		}
	}
	else {
		die "InputOutputError: unknown VCF format (.vcf or .vcf.gz)\n";
	}
}
if ($verbose) {
	print "InputOutputReport: VCF inputs: \t";
	if (defined $file_vcf) {print "VCF: $file_vcf\n";} else {print "VCF1: null\n";}
}
### input refvcf
if (defined $file_refvcf) {
	if (! -s $file_refvcf) {
		die "MainError: can not find REFVCF file: $file_refvcf\n";
	}
	if ($file_refvcf=~/\.vcf$/i) {
		if (! BgzipVcf($file_refvcf, "$file_refvcf.gz", $path_bgzip)) {
			die "InputOutputError: REFVCF bgzip error\n";
		}
		elsif (! IndexVcf("$file_refvcf.gz", $path_tabix)) {
			die "InputOutputError: REFVCF index error\n";
		}
		else {
			$file_refvcf.='.gz';
		}
	}
	elsif ($file_refvcf=~/\.vcf\.gz$/i) {
		unless (-s "$file_refvcf.tbi") {
			unless (IndexVcf($file_refvcf, $path_tabix)) {
				die "InputOutputError: REFVCF.gz index error\n";
			}
		}
	}
	else {
		die "InputOutputError: unknown REFVCF format (.vcf or .vcf.gz)\n";
	}
}
if ($verbose) {
	print "InputOutputReport: REFVCF inputs: \t";
	if (defined $file_refvcf) {print "VCF: $file_refvcf\n";} else {print "VCF1: null\n";}
}
...
#		print "Term: $read_num_arr[0]\t TotalReads: $read_num_arr[1]\n"; ### For test ###
		if ($read_num_arr[0] eq 'TotalAABB') {
			$totalreads=$read_num_arr[1];
			die "Error: wrong AABB read sum number\n" unless (defined $totalreads and $totalreads);
		}
		elsif ($read_num_arr[0] =~/^Total.*/) {
			next;
		}
		else {
...
# REF ploidy
my %refploidy=();
if (defined $file_ploidy and -s $file_ploidy) {
	close REFPLDY if (defined fileno(REFPLDY));
	unless (open (REFPLDY, "< $file_ploidy")) {
		die "Error: can not open --refploidy file\n";
	}
	while (my $refpld_line=<REFPLDY>) {
		chomp $refpld_line;
		my @refpld_arr=split(/\t/, $refpld_line);
		if (scalar(@refpld_arr)!=2) {
			print STDERR "Warnings: ignored line when reading refploidy: $refpld_line\n";
			next;
		}
		if (exists $refploidy{$refpld_arr[0]}) {
			die "Error: repeated cluster ID: $refpld_arr[0] in --refploidy $file_ploidy\n";
		}
		else {
			$refploidy{$refpld_arr[0]}=$refpld_arr[1];
		}
	}
	close REFPLDY;
	
	if (1) {
		foreach (sort {$a<=>$b} keys %refploidy) {
			print $_, "\t", $refploidy{$_}, "\n";
		}
	}
}
...
if (! -d "$RunDir/AABB") {
	mkdir ("$RunDir/AABB", 0766) || die "(Main0)Error: can not create folder $RunDir/AABB\n";
}
...
## Stage1: extract fasta reference
	$stage=1;
	print "(Main$stage)Step: Extract AABB fasta\n"; ### For test ###
...
###Stage2: Extract bam
	$stage=2;
	print "(Main$stage)Step: Extract AABB BAMs\n"; ### For test ###
	my $cluter_oribam="$RunDir/Clust$cluster_num/AABB.$cluster_num.ori.bam";
	unless (exec_cmd_return("bam_multiextract.sh -b $bamfiles -s $fastaids -n $numbams -o $cluter_oribam")) {
		print STDERR "(Main$stage)Error: AABB BAM extraction failed\n";
		print CLUSTERLOG $cluster_num."\tFail\t1\t$stage\tBAMExtract\n";
		next MAINPROG;
	}
	##ReadSam
	my ($test_readbam, $readbam_obj)=ReadSam($cluter_oribam, $clusterrefer, 1);
	unless ($test_readbam) {
...
##Stage3: Defined important hash or array here
	$stage=3;
	my $assignallele={};
...
### Stage4: ploidy
	$stage=4;
	my ($test_aa_expressed, $test_bb_expressed)=(0, 0);
	my $ploidy=0;
	my $inferred_genoploidy='';
	if (exists $refploidy{$cluster_num}) {
		if ($refploidy{$cluster_num}=~/A/) {
			$test_aa_expressed=1;
			$inferred_genoploidy.='A'
		}
		if ($refploidy{$cluster_num}=~/B/) {
			$test_bb_expressed=1;
			$inferred_genoploidy.='B'
		}
	}
	else {
		print STDERR "Warnings: NO REFploidy\n";
	}
	$ploidy=$test_aa_expressed+$test_bb_expressed;
	$inferred_genoploidy='N' if ($ploidy==0);
	print GENOLOG "Cluster $cluster_num Geno: $inferred_genoploidy\n";
	print GENOLOG "Cluster $cluster_num Ploidy: $ploidy\n";
	if ($ploidy==1) {
		goto GROUPREADBLK;
	}
	elsif ($ploidy==0) {
...
### Stage5: Extract AABBDD VCF
	$stage=5;
	my $run_assignread=0;
	print "(Main$stage)Step: Extract REF VCF\n"; ### For test ###
	my $cluster_refvcf="$RunDir/Clust$cluster_num/AABB.$cluster_num.ref.vcf.gz";
	unless (ExtractVcf($file_refvcf, $cluster_line, $cluster_refvcf)) {
		print STDERR "(Main$stage)Error: REFVCF extraction failed\n";
		print CLUSTERLOG $cluster_num."\tWarnings\t1\t$stage\tREFVcfExtract\n";
	}
	if (-s $cluster_refvcf) {
		$run_assignread=1;
	}
...
#								'ref' => chr => ++));
	my $readsum={};
	if ($run_assignread==1) {
		my ($test_loadrefvcf, $readrefvcf_obj)=ReadVcf($cluster_refvcf, 0);
		unless ($test_loadrefvcf) {
			print STDERR "(Main$stage)Error: load refvcf $cluster_refvcf failed\n";
			print CLUSTERLOG $cluster_num."\tFail\t1\t$stage\tLoadRefVCF\n";
			#next MAINPROG;
			goto DIRECTASSEM;
		}
		(my $test_assignreads, $readsum)=&AssignReads($readbam_obj, $readrefvcf_obj);
		if ($test_assignreads) {
			print STDERR "(Main$stage)Error: AssignReads SUB failed\n";
			print CLUSTERLOG $cluster_num."\tFail\t1\t$stage\tAssignReads\n";
			#next MAINPROG;
			goto DIRECTASSEM;
		}
	}
...
### Extract AABB vcf
	...
	my $cluster_orivcf="$RunDir/Clust$cluster_num/AABB.$cluster_num.ori.vcf.gz";
	unless (ExtractVcf($file_vcf, $cluster_line, $cluster_orivcf)) {
		print STDERR "(Main$stage)Error: VCF extraction failed\n";
...
###Stage7: Check Point: detect polymorphism, continue if no, region and merge if yes
	$stage=7;
	my $total_sites=0;
...
#Format: %{$polymorphicsites}=($chr => ($posit => ++));
	my $polymorphicsites={};
	(my $test_loadvcf, $readvcf_obj)=ReadVcf($cluster_orivcf, 20);
	unless ($test_loadvcf) {
	...
	foreach my $chrom (keys %{$readvcf_obj}) {
		foreach my $posit (keys %{${$readvcf_obj}{$chrom}}) {
			$total_sites++;
			if (exists ${$readvcf_obj}{$chrom}{$posit}[2]) {
				my $thisgeno=${$readvcf_obj}{$chrom}{$posit}[2];
				if ($thisgeno=~/\//) {
...
#					print "(Main$stage)Test: Chr:Pos $chrom:$posit Geno: $thisgeno NumAllele: ".scalar(keys %hash1)."\n"; ### For test ###
					...
					print STDERR "(Main$stage)Error: error genotypes: $thisgeno\n";
					print CLUSTERLOG $cluster_num."\tFail\t1\t$stage\tErrorGeno\n";
					delete ${$readvcf_obj}{$chrom}{$posit};
					next;
				}
			}
			else {
...
###Stage8: Group alleles into subgenome
##GroupReads
	GROUPREADBLK: {
	$stage=8;
	print "\n(Main$stage)Step: GroupReads\n"; ### For test ###
	(my $test_groupreads, $fragments, $readsum)=&GroupReads($readbam_obj, $readvcf_obj);
	if ($test_groupreads) {
...
###Stage9: checkpoint
	$stage=9;
	my $fragmentfile="$RunDir/Clust$cluster_num/AABB.$cluster_num.fragments";
...
##Stage10: print geno
	PRINTGENO: {
	$stage=10;
	print "\n(Main$stage)Step: geno print\n"; ### For test ###
	$num_excluded_reads=0;
	$num_shared_reads=0;
	$totalmappedreads=0;
	foreach my $ind_read (keys %{$readsum}) {
		if (exists ${$readsum}{$ind_read}{'excluded'} and ${$readsum}{$ind_read}{'excluded'}>0) {
...
###Stage11: correct genome alleles using diploid and genome assignemnts
	$stage=11;
...
###Stage12: Fill in genotypes accroding to the corrected alleles
	$stage=12;
...
#		print "(Main$stage)Test: Genome assignment ###\n"; print Dumper $assignallele; ### for test ###
		(my $test_fiv, $test_run_phase, my $fixedgeno_hashindex, my $fillinAlleles_hashindex)=&FillInVariations($assignallele,$readvcf_obj, $inferred_genoploidy, $corrected_sites, $alleledepth); ###checl Fillin $polymorphic
#		print "(Main$stage)Test: Genome assignment ###\n"; print Dumper $assignallele; ### for test ###
...
#		print "(Main$stage)Test: ### Genoypes Fillin ###\n"; print Dumper $fixedAllele_3fillin; print "\n"; ### For test fixedAllele_3fillin###
	...
	if (0) {
		foreach my $chrom (keys %{$readvcf_obj}) {
			...
			foreach my $posit (sort {$a <=> $b } keys %{${$readvcf_obj}{$chrom}}) {
				...
				my $printline=$cluster_num."\t".$chrom."\t".$posit."\t".${${${$readvcf_obj}{$chrom}}{$posit}}[2]."\t";
				#%ancestralalleles
				$printline.="Diploid: \t";
...
##COMMENT: Main($stage) SNP phasing
	...
	if ($test_run_phase) {
		my $hapcompassvcf="$RunDir/Clust$cluster_num/AABB.$cluster_num.hapcompass.vcf";
		my $phasedsolution="$RunDir/Clust$cluster_num/AABB.$cluster_num.phased";
...
###Stage15: Bam2fastq2
	...
	(my $test_bam2fastq, $fastqfiles, $rgfpkms, $cluster_fpkm)=Bam2FastQ2($cluter_oribam, $inferred_genoploidy, $readsum, $polymorphismreadids, "$RunDir/Clust$cluster_num/reads", 1, 0, 1, $path_samtools);
	if (! $test_bam2fastq) {
...
### Stage16: Bam2FastqProg2
	$stage=16;
	DIRECTASSEM: {
		$stage=16;
		$inferred_genoploidy='N';
		print "(Main$stage)Step: Extract AABB FastQs\n"; ### For test ###
		my $fastq_prefix="$RunDir/Clust$cluster_num/AABB.$cluster_num.bam2fastq";
		my ($test_b2fq, $fqfiles)=Bam2FastqProg($cluter_oribam, $fastq_prefix, $addcmd_bam2fastq, $path_bam2fastq);
		unless ($test_b2fq>0) {
			print STDERR "(Main$stage)Error: AABB Bam2Fastq failed\n";
			print CLUSTERLOG $cluster_num."\tFail\t1\t$stage\tBam2FastqProg\n";
			next MAINPROG;
		}
		
		my $fastqmerge="$RunDir/Clust$cluster_num/AABB.$cluster_num.bam2fastq.merge.fastq";
		unless (MergeBam2Fastq($fastqmerge, $fqfiles)) {
			print STDERR "(Main$stage)Error: AABB MergeBam2Fastq failed\n";
...
##Format $cluster_fpkm=('genomesize' => size, 
#							'cluster' => FPKM
#							'read_group' => (RG1 => ('global' => FPKM, 'A' => FPKM, 'B' => FPKM, 'D' => FPKM)
#							'allelic' => ('A' => FPKM, 'B' => FPKM, 'D' => FPKM)
#											RG2 => ...)
	PRINTFPKM: {
	if (1) {
		if (exists ${$cluster_fpkm}{'genomesize'}){
			print CLUSTFPKM "Clust$cluster_num\tCLUST\tGenomeSize:".${$cluster_fpkm}{'genomesize'}."\n";
		}
		else {
			print CLUSTFPKM "Clust$cluster_num\tCLUST\tGenomeSize:N\n";
		}
		if (exists ${$cluster_fpkm}{'cluster'}){
			print CLUSTFPKM "Clust$cluster_num\tClustGlobal\t".${$cluster_fpkm}{'cluster'}."\n";
		}
		else {
			print CLUSTFPKM "Clust$cluster_num\tClustGlobal\tN\n";
		}
		if (exists ${$cluster_fpkm}{'allelic'}) {
			foreach (sort (keys %{${$cluster_fpkm}{'allelic'}})) {
				print CLUSTFPKM "Clust$cluster_num\tAllelic\t$_\t".${$cluster_fpkm}{'allelic'}{$_}."\n";
			}
		}
		if (exists ${$cluster_fpkm}{'read_group'}) {
			foreach my $ind_rg (sort (keys %{${$cluster_fpkm}{'read_group'}})) {
				print CLUSTFPKM "Clust$cluster_num\tReadGroup\t$ind_rg";
				foreach my $indg (sort (keys %{${$cluster_fpkm}{'read_group'}{$ind_rg}})) {
					print CLUSTFPKM "\t$indg\t".${$cluster_fpkm}{'read_group'}{$ind_rg}{$indg};
				}
				print CLUSTFPKM "\n";
			}
		}
...
### (Main17) Trinity
	...
	foreach (sort keys %{$fastqfiles}) {
		...
		unless (RunFqTrinity(${$fastqfiles}{$_}, "$RunDir/Clust$cluster_num/Clust$cluster_num.$_.trinity.fasta", $trinity_addcmd, $path_trinity)) {
			...
			my $mira_tmp_dir="~/MIRA/Clust$cluster_num";
			if (-d $mira_tmp_dir) {
				exec_cmd("rm -rf $mira_tmp_dir/*");
			}
			else {
...
###Stage19: rename fasta
	$stage=19;
	print "\n(Main$stage)Step: FastaRenamer\n"; ### For test ###
	foreach (keys %fasta_assembled) {
		unless (RenameFasta($fasta_assembled{$_}, "$fasta_assembled{$_}.rename.fa", "Cluster${cluster_num}_${_}_", 9, "Geno=$inferred_genoploidy")) {
...
### Stage22: Ending
	$stage=22;
	print CLUSTERLOG $cluster_num."\tSucceed\t0\t$stage\t0\t$inferred_genoploidy\t$final_ploidy\n";
}
close CLUSTER;
close CLUSTERLOG;
close REFERFPKM;
...
##COMMENT: cleanheader
	my $EBtest_cleanheader=SamCleanHeader("$EBoutput.multiddhead.bam", "$EBoutput.withRG.bam", "$EBoutput.noRG.bam", $EBseq_obj,  $path_samtools);
	if ($EBtest_cleanheader==0) {
		...
		if ( -s "$EBoutput.noRG.bam" and -s "$EBoutput.withRG.bam") {
			...
			unless (SortBam("$EBoutput.withRG.bam", "$EBoutput.withRG.st.bam", $path_samtools, ' ')) {
			...
			unless (MoveFile("$EBoutput.withRG.st.bam", $EBoutput)) {
				print STDERR "${EBsubinfo}Error: BAM moving: $EBoutput.withRG.bam\n";
				return 0;
			}
			unless (IndexBam("$EBoutput", $path_samtools)) {
				print STDERR "${EBsubinfo}Error: BAM index running +RG error: $EBoutput\n";
				return 0;
			}
			unless (SortBam("$EBoutput.noRG.bam", "$EBoutput.noRG.st.bam", $path_samtools)) {
				print STDERR "${EBsubinfo}Error: BAM sort running +RG error: $EBoutput.noRG.bam\n";
				return 0;
			}
			unlink "$EBoutput.noRG.bam" if (-s "$EBoutput.noRG.bam");
			unless (MoveFile("$EBoutput.noRG.st.bam", $EBoutnogroup)) {
				print STDERR "${EBsubinfo}Error: BAM moving: $EBoutput.withRG.bam\n";
				return 0;
			}
			unless (IndexBam($EBoutnogroup, $path_samtools)) {
...
### &AssignReads($ReadSam_obj, $readvcf_obj)
### Global: $debug, $verbose, %genomealleles, @genstr
### Dependancy:&ReadVariantType, $min_mapq, $bam_genome_tag, $bam_chromo_tag
### Note:
sub AssignReads {
	my ($ARreadsamobj, $ARreadvcfobj)=@_;

	my $ARsubinfo="SUB(AssignReads)";
#Format: %ARreadidsum=(readid => ('gen' => ( 'A' => ++, 'B' => ++, 'D' => ++), 
#								'chr' => ( '1AL' => ++, '!BL' => ++, '2DL' => ++, ...),
#								'shared' => ++/0,
#								'mapped' => 1/0,
#								'ref' => Reference=> ++
...
#					);
	my %ARreadidsum=();
	my $ARtotalreads_unmapped=0;
	my $ARtotalreads_lowqual=0;

### read allele for each reads
	foreach my $ARchrom (keys %{$ARreadvcfobj}) {
		my @ARpositions=sort {$a<=>$b} (keys %{${$ARreadvcfobj}{$ARchrom}});
		next unless (exists $genomealleles{$ARchrom} or exists ${$refallelesABD}{$ARchrom});
		print $ARsubinfo, "Test: Number of variations on $ARchrom: ", scalar(@ARpositions), "\n" if ($debug or $verbose);
		foreach my $ARind_pos (sort {$a <=> $b} @ARpositions) {
			next unless (exists $genomealleles{$ARchrom}{$ARind_pos} or exists ${$refallelesABD}{$ARchrom}{$ARind_pos});
			print $ARsubinfo, "Test:\t".$ARchrom, "\t", "Pos: $ARind_pos\tRef:${${${$ARreadvcfobj}{$ARchrom}}{$ARind_pos}}[0]\tVar:${${${$ARreadvcfobj}{$ARchrom}}{$ARind_pos}}[1]\tGen:${${${$ARreadvcfobj}{$ARchrom}}{$ARind_pos}}[2]", "\n" if ($debug or $verbose);
			my $ARend_pos=$ARind_pos+length(${${${$ARreadvcfobj}{$ARchrom}}{$ARind_pos}}[0])-1;
			my @ARchr_alignments=();
			@ARchr_alignments= $ARreadsamobj->get_features_by_location(-seq_id => "$ARchrom", -start => "$ARind_pos", -end => "$ARend_pos");
			
			foreach my $ARind_align (@ARchr_alignments) {
				my $ARthisreadid=$ARind_align->name;
				next if ($ARind_align->flag & 0x0004);
				${$ARreadidsum{$ARthisreadid}}{'shared'}=0;# if ($ARnumallele>1);
				next unless ($ARind_align->qual >= $min_mapq);
				#$ARalelecode: this read contains which allele genotype (0(ref)/1/2/3/.)
				my $ARalelecode=ReadVariantType($ARind_pos, ${${${$ARreadvcfobj}{$ARchrom}}{$ARind_pos}}[0], ${${${$ARreadvcfobj}{$ARchrom}}{$ARind_pos}}[1], ${${${$ARreadvcfobj}{$ARchrom}}{$ARind_pos}}[2], $ARind_align, 1);
				next unless ($ARalelecode =~ /^\d+$/);
				foreach my $ARgenstr (@genstr) {
					if (exists $genomealleles{$ARchrom}{$ARind_pos} and exists $genomealleles{$ARchrom}{$ARind_pos}{$ARgenstr} and $genomealleles{$ARchrom}{$ARind_pos}{$ARgenstr}=~/^\d+$/ and $genomealleles{$ARchrom}{$ARind_pos}{$ARgenstr} eq $ARalelecode) {
						$ARreadidsum{$ARthisreadid}{'gen'}{$ARgenstr}++;
					}
					elsif (exists ${$refallelesABD}{$ARchrom}{$ARind_pos} and exists ${$refallelesABD}{$ARchrom}{$ARind_pos}{$ARgenstr} and ${$refallelesABD}{$ARchrom}{$ARind_pos}{$ARgenstr}=~/^\d+$/ and ${$refallelesABD}{$ARchrom}{$ARind_pos}{$ARgenstr} eq $ARalelecode) {
						$ARreadidsum{$ARthisreadid}{'gen'}{$ARgenstr}++;
					}
				}
			}
	...
	if (1) {
		print $ARsubinfo, "Test: total reads: ", scalar(keys %ARreadidsum), "\n";
		print $ARsubinfo, "Test: total reads unmapped: $ARtotalreads_unmapped\n";
		print $ARsubinfo, "Test: total reads low-quality: $ARtotalreads_lowqual\n";
	}
	return (0, \%ARreadidsum);
}



### Group SNPs based on read length
### &GroupReads($ReadSam_obj, $readvcf_obj)
### Global: $debug, $verbose
### Dependancy:&ReadVariantType, $min_mapq, $bam_genome_tag, $bam_chromo_tag
### Note:
sub GroupReads {
	my ($GRreadsamobj, $GRreadvcfobj)=@_;
...
###COMMENT: retrieve all alignments mapped to this reference sequence
		my @GRchr_alignments=$GRreadsamobj->get_features_by_location(-seq_id => "$GRchrom");
...
### read allele for each reads
	foreach my $GRchrom (keys %{$GRreadvcfobj}) {
		my @GRpositions=sort {$a<=>$b} (keys %{${$GRreadvcfobj}{$GRchrom}});
		print $GRsubinfo, "Test: Number of variations on $GRchrom: ", scalar(@GRpositions), "\n" if ($debug or $verbose);
		foreach my $GRind_pos (sort {$a <=> $b} @GRpositions) {
			print $GRsubinfo, "Test:\t".$GRchrom, "\t", "Pos: $GRind_pos\tRef:${${${$GRreadvcfobj}{$GRchrom}}{$GRind_pos}}[0]\tVar:${${${$GRreadvcfobj}{$GRchrom}}{$GRind_pos}}[1]\tGen:${${${$GRreadvcfobj}{$GRchrom}}{$GRind_pos}}[2]", "\n" if ($debug or $verbose);
#			my @GRallelearr=split(/\//, ${${${$GRreadvcfobj}{$GRchrom}}{$GRind_pos}}[2]);
#			my %GRallelehash=();
...
#				return 1;
#			}
			my $GRend_pos=$GRind_pos+length(${${${$GRreadvcfobj}{$GRchrom}}{$GRind_pos}}[0])-1;
			my @GRchr_alignments=();
			@GRchr_alignments= $GRreadsamobj->get_features_by_location(-seq_id => "$GRchrom", -start => "$GRind_pos", -end => "$GRend_pos");
			...
			foreach my $GRind_align (@GRchr_alignments) {
				...
				my ($GRalelecode, $GRcapture_qual)=ReadVariantType($GRind_pos, ${${${$GRreadvcfobj}{$GRchrom}}{$GRind_pos}}[0], ${${${$GRreadvcfobj}{$GRchrom}}{$GRind_pos}}[1], ${${${$GRreadvcfobj}{$GRchrom}}{$GRind_pos}}[2], $GRind_align, 2);
...
#		print "${BFQsubinfo}Test: readname: $BFQarr[0]\n"; ### For test ###
		...
		if (exists ${$BFQallelicreads}{$BFQarr[0]}) {
			$BFQis_allelic=1;
			my @BFQtemp_gens=();
			if (exists ${${$BFQallelicreads}{$BFQarr[0]}}{'genGetPolymorphicReads'}) {
