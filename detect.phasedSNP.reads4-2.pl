...
Requirements:
	Programs: hapcompass.jar hc2vcf.jar, gzip, gunzip, cat, zcat, 
			samtools, freebayes, vcf-tools, express
...
our ($reference, $file_cluster);
our ($file_bam_aabbdd, $file_bam_aabb, $file_bam_aa, $file_bam_dd);###filelist
our ($geno_delimiter, $min_share_alignments);###vcf format
our ($bam_genome_tag, $bam_chromo_tag);###Bam
our ($path_express, $express_frag_len_mean, $express_frag_len_stddev, $express_max_read_len);###express
our ($path_freebayes, $path_tabix, $freebayes_min_coverage, $freebayes_min_alternative_count, $min_mapq);###freebayes
our $path_samtools;###samtools
our ($path_hapcompassjar, $path_hc2vcf, $path_java);###HapCompass
our ($path_vcfmerge, $path_vcfconcat);#VCFtools
our ($cluster_log, $fpkm_log, $allele_log);###LOG
...
GetOptions(
	...
	"minalign:i" => \$min_share_alignments,
	...
	"compasspath:s" => \$path_hapcompassjar,
...
$express_max_read_len=100 unless (defined $express_max_read_len);
###Freebayes
...
#HapCompass
$path_hapcompassjar="$RootDir/utils/hapcompass/hapcompass.jar";
$path_hc2vcf="$RootDir/utils/hapcompass/hc2vcf.jar";
$path_java='java';
#VCFtools
$path_vcfmerge='vcf-merge';
$path_vcfconcat='vcf-concat';
...
##COMMENT: Call SNP and prepahse
	...
	if (&exec_cmd_return("gunzip -c Clust$cluster_linenum/AABBDD.$cluster_linenum.vcf.gz > Clust$cluster_linenum/AABBDD.$cluster_linenum.vcf")) {
		print STDERR "(Main5)Error: gunzip uncompress Clust$cluster_linenum/AABBDD.$cluster_linenum.vcf.gz failed\n";
		print CLUSTERLOG $cluster_line."\tFail\t1\tFreeBayesAABBDDgunzip\n";
		next;
	}
	elsif (! -s "Clust$cluster_linenum/AABBDD.$cluster_linenum.vcf") {
		print STDERR "(Main5)Error: gunzip output Clust$cluster_linenum/AABBDD.$cluster_linenum.vcf.gz failed\n";
...
##COMMENT: Group alleles into subgenome
	...
	my %FixedAllele_1rnaseq=%{$rnaseq_allele_hashindex};
	my %FixedAllele_2genome=%{$fixed_allele_hashindex};
...
##COMMENT: Fill in blanks and prepare vcf for
	...
	if ($ploidy==1) {
		...
		%FixedAllele_3fillin=%{$fixed_allele_hashindex};
	}
	elsif ($ploidy==3 or $ploidy==2 or $ploidy==0) {
		(my $test_fiv, $test_run_HapCompass, my $fixedgeno_hashindex, my $fillinAlleles_hashindex)=&FillInVariations($fixed_allele_hashindex,$aabbdd_vcf_obj, $geno_ploidy);
	...
	if (1) {
		foreach my $chrom (keys %{$aabbdd_vcf_obj}) {
			...
			foreach my $posit (keys %{${$aabbdd_vcf_obj}{$chrom}}) {
				...
				my $printline=$cluster_line."\t".$chrom."\t".$posit."\t";
				#%FixedAllele_1rnaseq
				...
				$printline.="\t";
				#%fixedGeno_4fillin
				$printline.=(exists ${$fixedGeno_4fillin{$chrom}}{$posit}) ? ${$FixedAllele_3fillin{$chrom}}{$posit} : 'xxxxx';
				$printline.="\t";
				#$test_run_HapCompass
...
##Format: %GVfixAlleles=(chr => (pos => (A => allele/?, B=> allele/?, D => allele/?)))
	my %FIVfillin_alleles=%{$FIVfixallele_hashindex};
