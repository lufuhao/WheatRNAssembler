...
use FuhaoPerl5Lib::VcfKit qw/ExtractVcf ReadVcf RunFreebayes ReadVariantType HapcompassVcf HapcompassFragments RunHapCompass/;
...
### detect polymorphism, continue if no, region and merge if yes
	...
	foreach my $chrom (keys %{$aabbdd_vcf_obj}) {
		foreach my $posit (keys %{${$aabbdd_vcf_obj}{$chrom}}) {
			$total_sites++;
			if (defined ${$aabbdd_vcf_obj}{$chrom}{$posit}[2]) {
				if (${$aabbdd_vcf_obj}{$chrom}{$posit}[2]=~/\//) {
					my @arr=split(/\//, ${$aabbdd_vcf_obj}{$chrom}{$posit}[2]);
					my %hash1=();
					map {$hash1{$_}++} @arr;
...
#Format: %GVreadidsum=(readid => ('gen' => ( 'A' => ++, 'B' => ++, 'D' => ++), 
#								'chr' => ( '1AL' => ++, '!BL' => ++, '2DL' => ++, ...),
#								'shared' => ++/0,
#								'mapped' => ++/0,
#								'ref' => chr => ++
...
#Format: %{$readids_by_alleles}=(chr => (pos => (allele1 => @readIDs, allele2 => @readids)))
	...
	(my $test_groupvcf, my $assignallele, my $fragments_hashindex, my $readsum, my $readids_by_alleles, $test_bb_expressed)=&GroupVcf($aabbdd_bam_obj, $aabbdd_vcf_obj);
	if ($test_groupvcf) {
...
#	my %FixedAllele_2genome=%{$assignallele}; undef $assignallele;
	if (0) { ### For test ###
		print "### (Main7) Genome assignment ###\n";
...
#Format: %fragments=(readid => (chrom => (pos => (geno1 => quality, geno2 => quality)));
	...
	my $error_notpolymorphic=0;
	CHECKGENO: {foreach my $chrom (sort (keys %{$aabbdd_vcf_obj})) {
		my @posits=keys %{${$aabbdd_vcf_obj}{$chrom}};
		foreach my $pos (sort {$a <=> $b} @posits) {
			my $thisgeno=${$aabbdd_vcf_obj}{$chrom}{$pos}[2];
			my @arrgeno=split(/:/, $thisgeno);
			unless ($arrgeno[0]=~/^\d+\/\d+\/\d+$/) {
				print STDERR "(Main7)Error: unknown geno: Chr:Pos: $chrom:$pos geno: $arrgeno[0]\n";
				$error_notpolymorphic=1;
				last CHECKGENO;
			}
			my @arrtypes=split(/\//, $arrgeno[0]);
			my %morphisms=();
			map {$morphisms{$_}++} @arrtypes;
			print "(Main7)Test: Chr:Pos: $chrom:$pos geno: $arrgeno[0]\n";
			my @morphism=keys %morphisms;
			if (scalar(@morphism)==1 and $morphism[0]=~/^\d+$/) {
				if (exists ${$readids_by_alleles}{$chrom} and exists ${${$readids_by_alleles}{$chrom}}{$pos} and ${${${$readids_by_alleles}{$chrom}}{$pos}}{$morphism[0]}) {
					print "(Main7)Info: Chrom:Pos $chrom:$pos no phlymorphism: @morphism\n";### For test ###
					foreach (@{${${${$readids_by_alleles}{$chrom}}{$pos}}{$morphism[0]}}) {
						${${$readsum}{$_}}{'shared'}=1;
					}
#					print "Main(7)Info: Deleting %{$assignallele} keys: Chrom:Pos: $chrom:$pos\n";### For test ###
					delete ${${$assignallele}{$chrom}}{$pos};
#					if (exists ${$assignallele}{$chrom}} and exists ${${$assignallele}{$chrom}}{$pos}) {### For test ###
...
#					}
	...
	if ($error_notpolymorphic==1) {
		print STDERR "(Main7)Error: check polumorphism\n";
		print CLUSTERLOG $cluster_num."\tFail\t1\tMain7_Check_Polymorphism\n";
		next;
	}
	if (0) {
		print "(Main7)Info: number of sites to be phased: $num_sites_to_phase\n";
		print "(Main7)Info: Polymorphic site genome assigned\n";
		print Dumper $assignallele;
	}
	if (1) {
		foreach my $chrom (keys %{$aabbdd_vcf_obj}) {
			...
			foreach my $posit (sort {$a <=> $b } keys %{${$aabbdd_vcf_obj}{$chrom}}) {
				...
				if (exists ${$assignallele}{$chrom} and exists ${${$assignallele}{$chrom}}{$posit}) {
					$printline.=(exists ${$assignallele}{$chrom}{$posit}{'A'}) ? ${$assignallele}{$chrom}{$posit}{'A'} : 'undef';
					$printline.="\t";
					$printline.=(exists ${$assignallele}{$chrom}{$posit}{'B'}) ? ${$assignallele}{$chrom}{$posit}{'B'} : 'undef';
					$printline.="\t";
					$printline.=(exists ${$assignallele}{$chrom}{$posit}{'D'}) ? ${$assignallele}{$chrom}{$posit}{'D'} : 'undef';
					$printline.="\t";
				}
				else {
					$printline.='undef'."\t".'undef'."\t".'undef'."\t";
...
#temp
	$test_run_phase=1 if ($num_sites_to_phase>1);
	...
	if ($ploidy==1) {
		...
		if ($test_run_phase==0) {
			%final_geno=%{$assignallele};
		}
	}
	elsif ($ploidy==3 or $ploidy==2 or $ploidy==0) {
		print "(Main9)Info: run_phasing before fillin: $test_run_phase\n";
		if (0) { ### For test ###
			print "### Genome assignment ###\n";
			print Dumper $assignallele;
		}
		(my $test_fiv, $test_run_phase, my $fixedgeno_hashindex, my $fillinAlleles_hashindex)=&FillInVariations($assignallele,$aabbdd_vcf_obj, $inferred_genoploidy, \%polymorphic_sites);
		...
		print "(Main9)Info: run_phasing after fillin: $test_run_phase\n"; ### For test ###
	}
	else {
...
##COMMENT: Main(10) SNP phasing
	print "\n(Main10)Step: phasing\n"; ### For test ###
	if ($test_run_phase) {
		unless (HapcompassVcf($cluster_orivcf, "$RunDir/Clust$cluster_num/AABBDD.$cluster_num.hapcompass.vcf", \%fixedGeno_4fillin, \%polymorphic_sites)) {
...
#	%polymorphismreadids=(readid => ('gen' => ( 'A' => ++, 'B' => ++, 'D' => ++), 
	...
	if ($num_sites_to_phase>0 ) {
		foreach my $chrom (keys %final_geno) {
#Format: %{$assignallele}=(chr => (pos => (A => allele/?, B=> allele/?, D => allele/?)))		
			...
			foreach my $pos (keys %{$final_geno{$chrom}}) {
				...
				foreach my $ind_geno (@keys) {
					if ($ind_geno=~/^[ABD]{1}$/) {
						...
						if (exists ${$readids_by_alleles}{$chrom} and exists ${${$readids_by_alleles}{$chrom}}{$pos} and exists ${${${$readids_by_alleles}{$chrom}}{$pos}}{$ind_allele}) {
							foreach my $read_id (@{${${${$readids_by_alleles}{$chrom}}{$pos}}{$ind_allele}}) {
								${${$polymorphismreadids{$read_id}}{'gen'}}{$ind_geno}++;
								if (exists ${$readsum}{$read_id}) {
									${${$readsum}{$read_id}}{'shared'}=0;
								}
								else { ### For test ###
									print STDERR "Warnings: not existed readsum readID: $read_id\n";
								}
							}
						}
						else {
...
### (Main12) Bam2fastq2
	...
	my ($test_bam2fastq, $fastqfiles, $rgfpkms)=&Bam2FastQ2($cluter_oribam, $inferred_genoploidy, $readsum, \%polymorphismreadids, "$RunDir/Clust$cluster_num/reads", 1, 0, 1, $path_samtools);
	if (! $test_bam2fastq) {
...
#		$fasta_assembled{$_}="$RunDir/Clust$cluster_num/mira4.$_.fasta");
	...
	if ($test_trinity==1) {
		print STDERR "(Main13)Error: Trinity may partially failed\n";
		next;
	}
	unless (chdir $RunDir) {
...
#	my %GValleleGenome=%{$GVfixAlleles_index};
	my %GValleleGenome=();
...
#					);
	my %GVreadidsum=();
#Format: %GVpolymorphicsites=($chr => ($posit => ++));
	my %GVpolymorphicsites=();
#Format: %GVexcludedreads=($readid => ++, ...)
	my %GVexcludedreads=();
...
###COMMENT: retrieve all alignments mapped to this reference sequence
		...
		foreach my $GVind_align (@GVchr_alignments) {
			...
			my $GVthisreadid=$GVind_align->name;
			${$GVreadidsum{$GVthisreadid}}{'mapped'}++;
			${$GVreadidsum{$GVthisreadid}}{'shared'}++;
			$GVreadidsum{$GVthisreadid}{'ref'}{$GVchrom}++;
			
			next if (exists $GVreadidsum{$GVthisreadid});###Reduce mem
			my $GVtag_chrom=$GVind_align->get_tag_values("$bam_chromo_tag");
			while ($GVtag_chrom=~/(\d[ABDLS]{1,2})/g) {
				${${$GVreadidsum{$GVthisreadid}}{'chr'}}{$1}++;
			}
			my $GVgenome_tagvalue=$GVind_align->get_tag_values("$bam_genome_tag");
			if ($GVgenome_tagvalue=~m/A/i) {
				${$GVreadid2genome{$GVthisreadid}}{'A'}=1;
				${${$GVreadidsum{$GVthisreadid}}{'gen'}}{'A'}++;
			}
			if ($GVgenome_tagvalue=~m/B/i) {
				${$GVreadid2genome{$GVthisreadid}}{'B'}=1;
				${${$GVreadidsum{$GVthisreadid}}{'gen'}}{'B'}++;
			}
			if ($GVgenome_tagvalue=~m/D/i) {
				${$GVreadid2genome{$GVthisreadid}}{'D'}=1;
				${${$GVreadidsum{$GVthisreadid}}{'gen'}}{'D'}++;
			}
		}
	}

###
	foreach my $GVchrom (keys %{$GVaabbdd_vcfobj}) {
		...
		foreach my $GVind_pos (sort {$a <=> $b} @GVpositions) {
			...
			my @GVallelearr=split(/\//, ${${${$GVaabbdd_vcfobj}{$GVchrom}}{$GVind_pos}}[2]);
			my %GVallelehash=();
			map {$GVallelehash{$_}++} @GVallelearr;
			my $GVnumallele=keys(%GVallelehash);
			if ($GVnumallele<1 or $GVnumallele>3) {
				print STDERR "SUB(GroupVcf)Error: Chr:Pos $GVchrom:$GVind_pos: Number of allele <1 or >3\n";
				return 1;
			}
			my $GVend_pos=$GVind_pos+length(${${${$GVaabbdd_vcfobj}{$GVchrom}}{$GVind_pos}}[0])-1;
			my @GVchr_alignments=();
			@GVchr_alignments= $GVaabbdd_samobj->get_features_by_location(-seq_id => "$GVchrom", -start => "$GVind_pos", -end => "$GVend_pos");
			my $GVtotaldepth=0;
			my $GVassigndepth=0;
			my %GVreadcount=();
			my %GVassignedcount=();
			...
			${${$GValleleGenome{$GVchrom}}{$GVind_pos}}{'A'}='?';
			...
			${${$GValleleGenome{$GVchrom}}{$GVind_pos}}{'B'}='?';
			$GVdd_allele=$ancestralalleles{$GVchrom}{$GVind_pos}{'D'} if (exists $ancestralalleles{$GVchrom} and exists $ancestralalleles{$GVchrom}{$GVind_pos} and exists $ancestralalleles{$GVchrom}{$GVind_pos}{'D'});
			${${$GValleleGenome{$GVchrom}}{$GVind_pos}}{'D'}='?';
			print "SUB(GroupVcf)Test: Chr:Pos $GVchrom:$GVind_pos Allele AA: $GVaa_allele BB: $GVbb_allele DD: $GVdd_allele\n" if ($debug or $verbose);
			my %GVallele_assign=();
#Format: %GVallele_assign=('A' => (allele1 =>1, allele2 =>1), 'B' => (allele1 =>1, allele2 =>1), 'D' => (allele1 =>1, allele2 =>1))
			foreach my $GVind_align (@GVchr_alignments) {
				my $GVthisreadid=$GVind_align->name;
				next if ($GVind_align->flag & 0x0004);
				next unless ($GVind_align->qual >= $min_mapq);
				$GVreadcount{$GVthisreadid}++;
				...
				unless ($GVread_group=~/^\d{1,2}$/) {
					next;
				}
				if (exists ${$GVreadid2genome{$GVthisreadid}}{'A'} or exists ${$GVreadid2genome{$GVthisreadid}}{'B'} or exists ${$GVreadid2genome{$GVthisreadid}}{'D'}) {
					$GVassignedcount{$GVthisreadid}++;
					if (exists ${$GVreadid2genome{$GVthisreadid}}{'A'}) {
						${$GVallele_assign{'A'}}{$GVread_group}++;
					}
					if (exists ${$GVreadid2genome{$GVthisreadid}}{'B'}) {
						${$GVallele_assign{'B'}}{$GVread_group}++;
					}
					if (exists ${$GVreadid2genome{$GVthisreadid}}{'D'}) {
...
###For HapCompass fragments  file
					...
					if (exists ${${${$GVreadfragments{$GVthisreadid}}{$GVchrom}}{$GVind_pos}}{$GVread_group}) {
						if (ord($GVcapture_qual) > ord(${${${$GVreadfragments{$GVthisreadid}}{$GVchrom}}{$GVind_pos}}{$GVread_group})) {
							${${${$GVreadfragments{$GVthisreadid}}{$GVchrom}}{$GVind_pos}}{$GVread_group}=$GVcapture_qual;
						}
					}
					else {
						${${${$GVreadfragments{$GVthisreadid}}{$GVchrom}}{$GVind_pos}}{$GVread_group}=$GVcapture_qual;
					}
					push (@{${${$GVreadid_by_allele{$GVchrom}}{$GVind_pos}}{$GVread_group}}, $GVthisreadid);
					${$GVreadidsum{$GVthisreadid}}{'shared'}=0;
				}
			}
			$GVassigndepth=scalar(keys %GVassignedcount);
			$GVtotaldepth=scalar(keys %GVreadcount);
##COMMENT: retrieve best allele for each sungenome based on genome-info
			foreach my $GVsubgenome (keys %GVallele_assign) {
				...
				foreach my $GValleleat_pos (keys %{$GVallele_assign{$GVsubgenome}}) {
					print "SUB(GroupVcf)Test: $GVchrom:$GVind_pos Geno: $GVsubgenome Count: ${$GVallele_assign{$GVsubgenome}}{$GValleleat_pos}\n" if ($debug or $verbose);###for test###"
				...
				if ($GVsubgenome eq 'A') {
					if ($GVaa_allele=~/^\d+$/ or $GVbest_allele=~/^\d+$/) {
						if ($GVaa_allele eq $GVbest_allele or $GVbest_allele!~/^\d+$/) {
							${${$GValleleGenome{$GVchrom}}{$GVind_pos}}{'A'}=$GVaa_allele;
						}
						elsif ($GVaa_allele!~/^\d+$/) {
							${${$GValleleGenome{$GVchrom}}{$GVind_pos}}{'A'}=$GVbest_allele;
						}
					}
				}
				elsif ($GVsubgenome eq 'B') {
					${${$GValleleGenome{$GVchrom}}{$GVind_pos}}{'B'}=$GVbest_allele;
					if ($GVbb_allele=~/^\d+$/ or $GVbest_allele=~/^\d+$/) {
						if ($GVbb_allele eq $GVbest_allele or $GVbest_allele!~/^\d+$/) {
							${${$GValleleGenome{$GVchrom}}{$GVind_pos}}{'B'}=$GVbb_allele;
						}
						elsif ($GVbb_allele!~/^\d+$/) {
							${${$GValleleGenome{$GVchrom}}{$GVind_pos}}{'B'}=$GVbest_allele;
						}
				...
				elsif ($GVsubgenome eq 'D') {
					${${$GValleleGenome{$GVchrom}}{$GVind_pos}}{'D'}=$GVbest_allele;
					if ($GVdd_allele=~/^\d+$/ or $GVbest_allele=~/^\d+$/) {
						if ($GVdd_allele eq $GVbest_allele or $GVbest_allele!~/^\d+$/) {
							${${$GValleleGenome{$GVchrom}}{$GVind_pos}}{'D'}=$GVdd_allele;
						}
						elsif ($GVdd_allele!~/^\d+$/) {
							${${$GValleleGenome{$GVchrom}}{$GVind_pos}}{'D'}=$GVbest_allele;
						}
			...
			print "SUB(GroupVcf)Test: Chr:Pos $GVchrom:$GVind_pos VCF: ${${${$GVaabbdd_vcfobj}{$GVchrom}}{$GVind_pos}}[2] DiABD: $GVaa_allele/$GVbb_allele/$GVdd_allele Gen2ABD: ${${$GValleleGenome{$GVchrom}}{$GVind_pos}}{'A'}/${${$GValleleGenome{$GVchrom}}{$GVind_pos}}{'B'}/${${$GValleleGenome{$GVchrom}}{$GVind_pos}}{'D'}\n";# if ($debug or $verbose);###for test###
		}
	}
	return (0, \%GValleleGenome, \%GVreadfragments, \%GVreadidsum, \%GVreadid_by_allele, $GVbb_express); ### For HapCompass 20150621 ###
...
sub FillInVariations {
	my ($FIVfixallele_hashindex, $FIVvcf_obj, $FIVgenoploidy, $FIVpolymorphicstes)=@_;
...
#		print "DD\n"; ### For test ###
	...
	if ($FIVploidy<1 or $FIVploidy>3) {
		print "SUB(FillInVariations)Error: Total ploidy: $FIVploidy\n";# if ($debug or $verbose);
		return 1;
	}
...
##COMMENT: Fill in unknown alleles		
	FIVBLOCK1: {foreach my $FIVchrom (keys %{$FIVvcf_obj}) {
		...
		foreach my $FIVpos (@FIVpositions) {
			print "SUB(FillInVariations)Test: chrom:pos: $FIVchrom:$FIVpos\n" if ($debug or $verbose);
			print "SUB(FillInVariations)Test: @{${${$FIVvcf_obj}{$FIVchrom}}{$FIVpos}}\n" if ($debug or $verbose);
			unless (exists ${$FIVpolymorphicstes}{$FIVchrom} and exists ${$FIVpolymorphicstes}{$FIVchrom}{$FIVpos}) {
				print STDERR "SUB(FillInVariations)Test: Notpolymorphic site: chrom:pos: $FIVchrom:$FIVpos\n";# if ($debug or $verbose);
				next;
			}
			...
			my %FIVallele_count=%{$FIVallele_hashindex};
			...
			if ($FIVnum_allele_notsure==0) {
				push (@FIVarr_geno_fix, ${${${$FIVfillinalleles}{$FIVchrom}}{$FIVpos}}{'A'}) if ($FIVout_aa==1);
				push (@FIVarr_geno_fix, ${${${$FIVfillinalleles}{$FIVchrom}}{$FIVpos}}{'B'}) if ($FIVout_bb==1);
				push (@FIVarr_geno_fix, ${${${$FIVfillinalleles}{$FIVchrom}}{$FIVpos}}{'D'}) if ($FIVout_dd==1);
			}
				...
				if ($FIVout_aa==1) {
					...
					elsif (exists ${${${$FIVfillinalleles}{$FIVchrom}}{$FIVpos}}{'A'}) {
				...
				if ($FIVout_bb==1) {
					...
					elsif (exists ${${${$FIVfillinalleles}{$FIVchrom}}{$FIVpos}}{'B'}) {
				...
				if ($FIVout_dd==1) {
					...
					elsif (exists ${${${$FIVfillinalleles}{$FIVchrom}}{$FIVpos}}{'D'}) {
			...
			print "SUB(FillInVariations)Test: Fixed Alleles: ".${$FIVfixed_geno{$FIVchrom}}{$FIVpos}."\t$FIVchrom: $FIVpos\n";# if ($debug);
		}
	}}###FIVBLOCK1;
	return ($FIVtest_cmd, $FIVneed_runcompass, \%FIVfixed_geno, $FIVfillinalleles);
}
...
sub GetGenomeSize {
	...
	$GGSprefix='MyFastq' unless (defined $GGSprefix);
	...
	unless (exec_cmd_return("$GGSpath_kmergenie $GGSfastq $GGSaddcmd ")) {
		print STDERR "${GGSsubinfo}Error: invalid fastq input\n";
		return 0;
	}
